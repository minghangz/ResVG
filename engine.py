...
class data_prefetcher():
    ...
    def preload(self):
        try:
            samples, targets, feature = next(self.loader)
        ...
        with torch.cuda.stream(self.stream):
            ...
            self.next_feature = self.next_feature.to(self.device, non_blocking=True)
    ...
    def next(self):
        ...
        img, mask, target, feature = self.next_img, self.next_mask, self.next_target, self.next_feature
        self.preload()
        return img, mask, target, feature

    def __next__(self):
        img, mask, target, feature = self.next()
        if img == None:
            raise StopIteration
        return img, mask, target, feature
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    img, mask, target, feature = prefetcher.next()
    iteration = 0
    while img is not None:
        ...
        outputs = model(img, mask, word_id, word_mask, feature)
...
def train_one_epoch_w_accum(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    while img is not None:
        ...
        for i in range(2):
            ...
            outputs = model(b_img, b_mask, b_word_id, b_word_mask, feature)
...
def evaluate(model, criterion, postprocessor, data_loader, device, save_path=''):
    ...
    all_pred_boxes = []
    prefetcher = data_prefetcher(data_loader, device)
    for iteration, (img, mask, target, feature) in enumerate(tqdm(prefetcher)):
        target_dict = target.tensor_dict
        word_id, word_mask = target_dict['word_id'], target_dict['word_mask']
...
@torch.no_grad()
def validate(model, postprocessor, image, mask, target, device):
    model.eval()
    target_dict = target
    word_id, word_mask = target_dict['word_id'], target_dict['word_mask']
    img, mask, word_id, word_mask = image.to(device), mask.to(device), word_id.to(device), word_mask.to(device)
    img = img.unsqueeze(0)
